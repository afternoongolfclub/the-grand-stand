...
function _renderAuthModal() {
  ...
  $('auth-modal-title').textContent = isReset ? 'Reset Password' : isSignup ? 'Create Account' : 'Sign In';
  $('auth-google-btn').style.display  = isReset ? 'none' : '';
  document.querySelector('.auth-divider').style.display = isReset ? 'none' : '';
  $('auth-name-field').style.display  = isSignup ? '' : 'none';
  $('auth-password-field').style.display = isReset ? 'none' : '';
  $('auth-submit-btn').textContent    = isReset ? 'Send Reset Email' : isSignup ? 'Create Account' : 'Sign In';
  $('auth-toggle-btn').style.display  = isReset ? 'none' : '';
  $('auth-reset-btn').style.display   = isReset ? 'none' : '';
  $('auth-toggle-btn').innerHTML      = isSignup
    ? 'Already have an account? <strong>Sign In</strong>'
    : 'Don\'t have an account? <strong>Sign Up</strong>';
  // Clear fields
  ['auth-name','auth-email','auth-password'].forEach(id => { if($(id)) $(id).value = ''; });
}
...
function showResetPassword() {
  ...
  _renderAuthModal();
  // Show a back link
  $('auth-submit-btn').insertAdjacentHTML('afterend',
    '<button class="btn-link" style="margin-top:.5rem;display:block;text-align:center" onclick="toggleAuthMode()">← Back to Sign In</button>'
  );
}
...
async function submitAuthForm() {
  ...
  if (authMode === 'reset') {
    ...
    } catch(e) { showToast(_authError(e), 'error'); }
  ...
  try {
    if (authMode === 'signup') {
      if (!name) { showToast('Display name required', 'error'); setLoading(false); return; }
...
async function saveProfile(displayName) {
  ...
  await db.collection('users').doc(state.currentUser.uid).update({ displayName });
...
function updateAuthUI() {
  ...
  if (state.currentUser && state.userProfile) {
    ...
    el.innerHTML = `<button class="btn btn-gold btn-sm" onclick="openAuthModal()">Sign In</button>`;
  }
}
  ...
  if (!state.currentUser) {
    app().innerHTML = `
      ...
      <div class="sign-in-prompt">
        ...
        <button class="btn btn-google" onclick="openAuthModal()">
...
async function init() {
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
      const isNew = await ensureUserProfile(user);
      // Prompt new users (or those without a display name) to set username
      if (isNew || !state.userProfile.displayName) {
